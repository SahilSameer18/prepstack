...
- **User-Keyed Adaptive Rate Limiting:** Enforces rate limits keyed by authenticated `User ID` with Express `trust proxy` configuration, allowing multiple users on shared Wi-Fi networks to operate without false lockouts.
...
- **Secure Dual-Token Auth System:** Employs HTTP-only `SameSite` cookie pairs alongside `Authorization: Bearer <token>` payload support. Access tokens rotate on every `/refresh` call, checking against database records to prevent replay attacks.
...
| `PUT`  | `/change-password`   | ✅ JWT | Verifies current password and updates to new secure hash        |
